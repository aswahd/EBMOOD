...
if args.load != '':
    for i in range(1000 - 1, -1, -1):
        model_name = os.path.join(args.load, args.method_name + '_nets' + '_epoch_' + str(i) + '.pt')
...
ood_data = dset.ImageFolder(root="data/texture",
...
ood_data = svhn.SVHN(root='data/svhn/', split="train",
...
ood_data = lsun_loader.LSUN("data/LSUN",
                            transform=trn.Compose([trn.Resize(32), trn.CenterCrop(32),
                                                   trn.ToTensor(), trn.Normalize(mean, std)]))
ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)
...
if 'cifar10_' in args.method_name:
    ood_data = dset.CIFAR100('data/cifarpy', train=False, transform=trn.ToTensor())
else:
